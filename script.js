...
var robotWidth = 30; //inches
var robotHeight = 30; //inches
var pointRadius = 5;
var turnRadius = 30;
var kEpsilon = 1E-9;
var image;
...
class Waypoint {
	...
	toString() {
		...
		return "new Waypoint("+this.position.x+","+this.position.y+","+this.radius+","+this.speed+")" + comment;
	}

	flip() {
		var pos = new Translation2d(this.position.x, fieldHeight - this.position.y);
		var velo = this.speed;
		var rho = this.radius;
		var com = this.comment;
		return new Waypoint(pos, velo, rho, com);
	}
}
...
function addPoint() {
	...
	$("tbody").append("<tr>"
		+"<td><input value='"+(prev.x+30)+"'></td>"
		+"<td><input value='"+(prev.y)+"'></td>"
		+"<td><input value='0'></td>"
...
function getDataString() {
	...
	pathInit = pathInit.substring(0, pathInit.length -2) + "\n";
	var str = `
...
function flipAllWayPoints(listOwaypoints){
	var mWayPoints = listOwaypoints;
	for (var ind = 0; ind < mWayPoints.length; ind++){
		mWayPoints[ind] = waypoints[ind].flip();
	}
	return mWayPoints;
}